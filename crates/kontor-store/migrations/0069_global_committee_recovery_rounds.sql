-- Schema v69. Completion can reopen a terminal failed verdict through a later
-- identity-bound LSA proposal and distinct TPM route. That recovery produces a
-- new Committee run after the template's ordinary bounded rounds, so every
-- durable global round field must admit the same positive `u8` domain as the
-- scheduler. Keeping any one of these fields capped at two strands a completion
-- after its recovery integration or on a later explicit recovery.

-- SQLite cannot widen a CHECK constraint in place. Rebuild the run table while
-- preserving every column, reference, index and frozen-input trigger. The
-- migrator disables foreign-key enforcement around the transaction and checks
-- the complete graph before commit, so existing child rows remain attached to
-- the replacement table under the same canonical name.
...
-- This insert guard lives on the Advisor artifact table but reads the run
-- table. SQLite reparses trigger bodies while the table is rebuilt, so remove
-- and restore it around the canonical-name gap as part of the same migration.
...
CREATE TABLE epic_completion_remediation_command_claims_v69 (
    project_id      TEXT    NOT NULL REFERENCES projects(id) ON DELETE RESTRICT,
...
INSERT INTO epic_completion_remediation_command_claims_v69
    (project_id, mini_project_id, round, action, idempotency_key, intent_hash,
     effect_revision, claimed_at)
SELECT project_id, mini_project_id, round, action, idempotency_key, intent_hash,
       effect_revision, claimed_at
FROM epic_completion_remediation_command_claims;

DROP TABLE epic_completion_remediation_command_claims;
ALTER TABLE epic_completion_remediation_command_claims_v69
