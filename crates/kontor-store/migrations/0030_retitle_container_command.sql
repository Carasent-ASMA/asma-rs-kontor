-- The container retitle command.
--
-- One change, and it is the only durable state this operation needs: a receipt
-- kind. The title itself is not stored, because it is not Kontor's to remember —
-- it is derived from the node's pinned topology and the plane's typed scope at
-- the moment the operation runs, and the runtime is read back for what the
-- container actually carries. A stored copy would be a second answer to a
-- question the runtime answers authoritatively, and it would go stale the first
-- time anything renamed a workspace outside this operation.
--
