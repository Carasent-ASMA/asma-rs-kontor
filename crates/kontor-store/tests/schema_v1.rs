...
fn an_empty_database_migrates_to_the_current_schema_version() {
    ...
    // v106 makes one Core Team route succession recoverable across the
    // interval between its committed store transition and its command
    // receipt, so a process lost in that interval converges on the
    // occupant it already installed instead of refusing its own effect
    // (ASMA-8187).
