...
fn a_document_may_only_claim_the_quota_chain_its_generation_had() {
    ...
    for refused in [48, 51, 62, 73] {
        assert!(
            ...
            "a generation-three document over database {refused} must not read as complete",
        );
    }
    ...
    at_database(&current, 74).expect("the generation that holds the whole chain verifies");
}
