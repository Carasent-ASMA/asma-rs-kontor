...
/// Export generation that first defined profile-selection outcomes.
///
/// Pinned rather than written as "the current generation": these checks ask
/// whether *this feature* was defined yet, and comparing against
/// `EXPORT_SCHEMA_VERSION` silently rejects every previously valid document the
/// moment the generation is bumped for an unrelated feature.
...
///
/// Staged as 0074 on this branch; integration renumbers it to **0077** (OP-22
/// ends at schema max 76 after its own renumbering), and this constant moves
/// with it. Used for the provenance-specific invariant only.
const QUOTA_OBSERVATION_PROVENANCE_SCHEMA_VERSION: i64 = 74;

/// Export generation that first defined the quota chain.
const QUOTA_OBSERVATION_PROVENANCE_EXPORT_VERSION: u32 = 4;
