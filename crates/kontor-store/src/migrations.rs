...
const MIGRATIONS: &[&str] = &[
    ...
    // Schema v67. Immutable provider-usage observations prove freshness even
    ...
    // Schema v74. Exact link recovery adopts its original pending create batch
    // and records recovery intent before any connector readback.
    include_str!("../migrations/0074_in_place_jira_materialization_recovery.sql"),
    ...
    // Schema v79. The receipt a confirmed migration was commanded under, so a
    ...
    // Schema v100. A revoked authorization may carry the condition that would
    // end it, so a hold states what lifts it instead of waiting on a human to
    // re-read its prose.
    include_str!("../migrations/0100_hold_lift_conditions.sql"),
];
