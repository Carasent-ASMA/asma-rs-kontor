...
> **Category:** report  
> **Amended:** 2026-08-19 19:37 CEST — OP-12 inspector lineage flag  
...
## OP-12 inspector flag disposition

The inspector evidence is accepted in part. Direct comparison confirms:

- OP-12's merged `apply_pending` hardcodes the operational-hardening
  convergence list through `MIGRATIONS[40]`;
- OP-08 already owns the general correction: enumerate every migration from
  `CONSULTATION_GENERATION`, skipping only `ESCALATION_GENERATION`; and
- OP-12's QA report proves
  `the_operational_hardening_v35_lineage_converges_without_losing_its_receipt`
  is the test that detects a stale convergence list, while a fresh database
  cannot.

The builder must therefore resolve the future `migrations.rs` conflict in
OP-08's favour and keep the self-extending loop. The hardcoded OP-12 list is
subsumed and must be dropped.

The inspector's request to assign OP-08 `0042` and `MIGRATIONS[41]` is rejected
as superseded, not ignored. It reflects the ordering recorded when PR #45 was
reviewed; the later binding LSA correction reserves OP-14 / ASMA-7941 as
`0042` / `MIGRATIONS[41]`. OP-08 remains
`0043_project_subject_authority.sql` / `MIGRATIONS[42]`, with
`SCHEMA_VERSION == 43`.

During that integration, `schema_v1.rs::EXPECTED_TABLES` must start from the
exact post-OP-14 master inventory, retaining OP-12's four `open_question*`
tables and all OP-14 tables, then add OP-08's three authority tables:
`project_subject_authority`, `subject_import_manifests` and
`subject_authority_receipts`. The schema assertion moves to 43, never 42.

Required narrow gate, in addition to the workspace suite:
...
Its evidence must show the historical v35 database reaches schema 43 and keeps
the existing receipt. This is a future post-OP-14 implementation gate; this
architecture amendment does not rename or alter a migration.
...
4. resolve `migrations.rs` by retaining the self-extending loop and dropping
   master's hardcoded list;
5. renumber local authority directly to `0043` / `MIGRATIONS[42]`, set schema
   43 and merge `EXPECTED_TABLES` as the full OP-12 + OP-14 + OP-08 union; and
6. run fresh-v42, populated-v42, operational-hardening-v35-to-v43,
   restart/replay and full store/OpenAPI parity gates before committing or
   pushing.
