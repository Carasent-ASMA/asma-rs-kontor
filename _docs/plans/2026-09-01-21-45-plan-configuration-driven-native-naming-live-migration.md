...
- one exact `(container kind, RoleSlotId)` resolver covers both fixed local
  `slots` and TeamRun-supplied `team_slots`; absence is a pre-effect refusal and
  no caller role can become a fallback.
- the recommended ECP registers the Core Team's actual deterministic slot ids
  exactly as `lsa→LSA` and `tpm→TPM`; configuration, storage and runtime launch
  therefore use one address rather than translating from prose aliases.
- upgrade preview preflights every slot of every open TeamRun on active topology
  against the exact target definition before the first runtime read. An open
  run whose exact seats and node are retired remains historical evidence.
  Alternative templates may share catalog codes, but the actual active run may
  not contain a rendered collision.
- an in-flight migration fences delivery admission, topology materialization
  seat replacement, seat release and every node lifecycle transition before
  any durable command, predecessor retirement, lifecycle write or runtime
  effect. The store check and lifecycle write share one immediate transaction.
- both migration record and confirmation require an exact bidirectional census:
  every live subject and immutable native identity is present, and no stale
...
- export generations 2 and 3 normalize absent generation-4 Team Definition
  arrays only in memory. Hashing, continuity and canonical serialization retain
  the exact older-generation shape, proven by a fixture emitted by the exact
  generation-3 predecessor binary.
...
semantic mutants cover and kill all three. A fresh audit of `718e5fd` found
three further P1s: incomplete exact-slot authority/target preflight, a late
admission-and-replacement fence with one-way census proof, and claimed
generation-3 export support that still required the generation-4 record shape.
Those findings now also have focused red/green regressions and killed semantic
mutants. A fresh audit of `12ca30b` then rejected three historical-lifecycle
edges: an inactive exact delivery seat still failed the census, archived nodes
still entered TeamRun preflight and kind-based parent resolution, and lifecycle
writes could race a frozen migration census. The remediation now distinguishes
proven historical seats from genuinely seatless active runs, follows the exact
active `parent_id`, skips inactive nodes during preflight, and serializes every
seat/node lifecycle write against the migration record. The remaining gap is
the exact-head archive gate, a fresh passing audit, and operational release:
merge the verified artifacts, deploy exact master, reconcile the live KBI
topology, and apply/read back the live KBI migration.
...
| Skip target-definition preflight of the live TeamRuns before upgrade preview. | Missing/colliding target mapping zero-runtime-call preview test. | Killed; test failed before restoration |
| Let replacement bypass the in-flight Team Definition migration fence. | Replacement preserves the predecessor byte-for-byte, records no command and contacts no runtime. | Killed; test failed before restoration |
| Compare only migration subjects and ignore immutable native identities or stale extra subjects. | Bidirectional subject-and-identity census test. | Killed; test failed before restoration |
| Deserialize generation 3 without generation-aware record normalization. | Exact export fixture generated by the predecessor generation-3 binary. | Killed; test failed before restoration |
...
- `cargo clippy --workspace --all-targets --locked -- -D warnings`: passed after the second-audit remediation.
- Four second-audit semantic mutants were each introduced separately, killed by
  their focused regression, and restored; all four focused regressions then
  passed in the unmutated tree.
- `cargo test -p kontor-daemon --test loopback_api`: 241 expected green tests;
  the first full run exposed four ECP slot-address fixture mismatches, all four
  passed after the definition was corrected to `lsa`/`tpm`; the fifth failure
  was sandbox port denial and its exact test passed with loopback permission.
  One pre-existing superseded Jira test remains intentionally ignored. The
  clean-commit archive gate below is the authoritative full rerun.
...
- The third-audit remediation passes all 13 migration-completeness persistence
  tests, all 5 Team Definition daemon tests, the three focused historical/fence
  black-box tests, and workspace clippy with warnings denied. The exact-head
  archive gate and fresh independent verdict remain the merge authority.
...
- Retired or archived topology nodes and inactive seats are outside the live migration census and retain their historical native names. Lifecycle must first be reconciled through supported operations; active work is never retired merely to make preview pass.

## 9. Completion rule

Complete only when all RED contracts are green, every recorded mutant is killed, the exact committed archive gate passes, the delivery PR is merged, exact master binaries are deployed, schema v80 is healthy, the recommended definition is explicitly published/selected and pinned, every open live KBI TeamRun has its calibrated delivery SeatBindings reconciled, completed or detached TeamRuns remain historical, every active KBI native ID is preserved with its exact new name read back, the high-stakes TeamRun artifacts/gates close, Jira `ASMA-8062` and the Kontor task are complete, and the implementation worktree is removed cleanly.
