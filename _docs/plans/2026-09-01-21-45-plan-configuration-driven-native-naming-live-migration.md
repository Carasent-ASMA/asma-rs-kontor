...
- upgrade preview preflights every slot of every live TeamRun against the exact
  target definition before the first runtime read. Alternative templates may
  share catalog codes, but the actual run may not contain a rendered collision.
- an in-flight migration fences delivery admission, topology materialization
  and seat replacement before any durable command, predecessor retirement or
  runtime effect.
...
mutants. The remaining gap is archive
